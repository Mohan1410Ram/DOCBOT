...
df_tr = pd.read_csv('E:/DOCBOT/Medical_dataset/Training.csv')
df_tt = pd.read_csv('E:/DOCBOT/Medical_dataset/Testing.csv')
...
# load model
knn_clf = joblib.load('E:/DOCBOT/model/knn.pkl')
...
def getDescription():
    ...
    with open('E:/DOCBOT/Medical_dataset/symptom_Description.csv') as csv_file:
...
def getSeverityDict():
    ...
    with open('E:/DOCBOT/Medical_dataset/symptom_severity.csv') as csv_file:
...
def getprecautionDict():
    ...
    with open('E:/DOCBOT/Medical_dataset/symptom_precaution.csv') as csv_file:
        csv_reader = csv.reader(csv_file, delimiter=',')
        line_count = 0
        for row in csv_reader:
...
def calc_condition(exp, days):
    ...
    if ((sum * days) / (len(exp)) > 13):
        ...
        print("It would be better if you follow precautionary measures.")
        return 0
...
def related_sym(psym1):
    ...
    if num != 0:
        s += "Select what you meant from the following:"
        return s
    else:
        return 0
...
def get_bot_response():
    ...
    if "step" in session:
        if session["step"] == "Q_C":
            ...
            session.clear()
            if s == "q":
                "Thank you for using CHATBOT_DOCTOR Mr/Mrs " + name
            else:
                session["step"] = "FS"
                session["name"] = name
                session["age"] = age
    ...
    if session['step'] == "Depart":
        ...
        return "Well, Hello again Mr/Ms " + session[
            "name"] + ", now I will ask few questions related to symptoms and decide what you should do. Reply yes to start diagnostication!"
    if session['step'] == "BFS":
        session['step'] = "FS"  # first symp
        return "Please mention your main symptom Mr/Ms " + session["name"] + "."
    if session['step'] == "FS":
        sym1 = s
        ...
        if sim1 == 1:
            ...
            return "You are probably facing another symptom, if so, please mention it."
    if session['step'] == "RS1":
        temp = session['FSY']
        psym1 = temp[2]
        psym1 = psym1[int(s)]
    ...
    if session['step'] == "END":
        if result is not None:
            if result[0] != session["testpred"]:
                session['step'] = "Q_C"
                session.clear()
                return "As you provide me with few symptoms, I am sorry to announce that I cannot predict the " \
                       "disease you're suffering from, for the moment!!!"
    ...
    if session['step'] == "Description":
        ...
        if session["disease"] in description_list.keys():
            ...
            return "Please visit <a href='" + "https://en.wikipedia.org/wiki/" + session["disease"] + "'> for detailed view of the disease. </a>"
    if session['step'] == "Severity":
        session['step'] = 'FINAL'
        if calc_condition(session["all"], int(s)) == 1:
            return "You should consult a doctor.<br> Type q to exit."
        else:
            msg = 'Nothing to worry about, kindly follow precautionary measures:<br> '
            i = 1
    ...
    if session['step'] == "FINAL":
        session['step'] = "BYE"
        return "Your diagnostication was perfectly completed. Do you need another medical assistance (reply yes or no)? "
    ...
    if session['step'] == "BYE":
        ...
        session.clear()
        if s.lower() == "yes":
            # If the user wants another medical assistance
            session["name"] = name
            session["age"] = age
            session["gender"] = gender
            session['step'] = "FS"
            session.clear()  # Clear the session to start a new conversation
            return "Hello again Mr/Ms"+session["name"]+"! Please tell me your main symptom."
        else:
            # If the user wants to exit
            name = session.get("name", "user")
            session.clear()  # Clear the session
            return f"Thank you Mr/Ms {name}, for using our chatbot service. Goodbye, Takecare!"
